...
setInterval(() => {
  http.get(`https://glitch.com/~ferrari-games`);
}, 280000);
    ...
    if(message.author.bot) return;
            if (!points[message.author.id]) points[message.author.id] = {
             points: 0,id: message.author.id
              ...
              if (message.content.startsWith(prefix + "فكك.ف.fkk.f") || message.content.startsWith(prefix+"فكك.ف.fkk.f")) {
                if(!message.channel.guild) return message.reply('**هذا الأمر للسيرفرات فقط**').then(m => m.delete(3000));

              const type = require("./فكك.ف.fkk.f.json");
           ...
           let author = message.author;
              ...
              message.channel.send("**لديك __15__ ثانية لتفكيك الكلمة**").then(msg => {
...
client.on("message", message => {
  ...
  if (!message.channel.guild) return;
        let args12 = message.content
            ...
            .slice(1)
            .join(" ");
        if (!message.author.id === "610607146173857826") return;
...
client.on('message' , message => {
  if(message.author.bot) return;
  if(message.content.startsWith(prefix + "xo","اكس","اكس او","o","x")) {
...
🎮 - ${prefix}rps
---------------------------------**`)
.setFooter('Copy Right Reserved For: خالد#7526')
